...
tt.layout$nodes$colour <- pal[c(as.character(metasub1$pango.group), # tips
                                NA, # root has no colour
                                anc)]  # internal nodes

# propagate colours to edges
tt.layout$edges$colour <- tt.layout$nodes$colour[tt.layout$edges$child]

fieldnames <- c("province", "host.gender", "host.age.bin", "sample.collection.date", 
                "sample.collected.by", "purpose.of.sampling", "purpose.of.sequencing",
                "lineage", "pango.group")
for (field in fieldnames) {
  temp <- rep(NA, nrow(tt.layout$edges))
  temp[tt.layout$edges$isTip] <- as.character(metasub1[[field]])
...
v.edges <- t(sapply(split(tt.layout$edges, tt.layout$edges$parent), function(e) {
  ...
  c(parent=NA, child=NA, length=NA, isTip=NA, 
    ...
    colour=e[1,]$colour, sample.collected.by=NA, sample.collection.date=NA,
